...
const VIEW_COLUMNS = {
  core: ["sequence", "status", "code", "codeChange", "parentCode", "simpleStatus", "name", "stock", "processingStock", "availableStock", "inboundDate", "inboundQty", "updatedAt", "note", "history"],
  stock: ["sequence", "status", "code", "codeChange", "parentCode", "simpleStatus", "name", "stock", "processingStock", "availableStock", "inboundDate", "inboundQty", "updatedAt"],
  catalog: ["sequence", "status", "code", "codeChange", "parentCode", "simpleStatus", "name", "stock", "processingStock", "availableStock", "inboundDate", "inboundQty", "updatedAt", "note", "history"],
...
function restoreLogin() {
  const user = localStorage.getItem(LOGIN_KEY) || localStorage.getItem(AUTHOR_KEY) || "";
  setLoginUser(user);
}
...
function setLoginUser(name) {
  ...
  if (user) {
    ...
    localStorage.setItem(AUTHOR_KEY, user);
  } else {
    localStorage.removeItem(LOGIN_KEY);
  }
  els.authorInput.value = user || "";
  els.loginBtn.textContent = user ? `${user} 로그아웃` : "로그인";
  els.loginBtn.classList.toggle("primary", !user);
  els.loginBtn.classList.toggle("subtle", Boolean(user));
}

function requireLogin() {
  if (getLoginUser()) return true;
  openLoginModal("수정하거나 파일을 등록하려면 먼저 로그인해주세요.");
  return false;
}
...
function updateLoginLockedControls() {
  ...
  [els.inventoryImportBtn, els.orderImportBtn, els.scheduleImportBtn].forEach((button) => {
    if (!button) return;
    button.disabled = locked;
    button.title = locked ? "로그인 후 사용할 수 있습니다." : "";
  });
  document.querySelectorAll("[data-code-change-input], [data-note-input], [data-parent-code-toggle]").forEach((control) => {
    control.disabled = locked;
    control.title = locked ? "로그인 후 수정할 수 있습니다." : "";
  });
}
...
function handleLoginSubmit(event) {
  ...
  if (!LOGIN_USERS[name] || LOGIN_USERS[name] !== password) {
    els.loginMessage.textContent = "이름 또는 비밀번호가 맞지 않습니다.";
    return;
  }
  setLoginUser(name);
  closeLoginModal();
