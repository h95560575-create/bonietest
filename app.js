...
function cleanItem(item) {
  ...
  return {
    ...
    autoZeroHidden: Boolean(item.autoZeroHidden),
...
function renderCell(item, column, rowNumber = 0) {
  ...
  if (column.key === "note") {
    return `
      <td class="note-cell">
        <textarea data-note-input data-code="${escapeHtml(item.code)}" rows="2" placeholder="">${escapeHtml(item.note || "")}</textarea>
      </td>
    `;
  }
...
function getCurrentStockValue(item, key) {
  if (key === "availableStock") return item.stock - item.processingStock;
  if (key === "processingStock") return item.processingStock || 0;
  return item.stock || 0;
}
...
function applyInventoryImport(table, fileName, importedCodes = null, importContext = null) {
  ...
  table.records.forEach((record) => {
    ...
    const payload = {
      ...
      autoZeroHidden: shouldAutoHideZeroStock(stock, processingStock),
      source: fileName,
...
function shouldAutoHideZeroStock(stock, processingStock) {
  const currentStock = toInteger(stock, 0);
  const currentProcessingStock = toInteger(processingStock, 0);
  return currentStock === 0 && currentProcessingStock === 0 && currentStock - currentProcessingStock === 0;
}
